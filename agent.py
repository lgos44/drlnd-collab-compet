...
class Agent():
    ...
    def __init__(self, config):
        ...
        self.actor_local = Actor(config.state_size, config.action_size, config.random_seed).to(device)
        self.actor_target = Actor(config.state_size, config.action_size, config.random_seed).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=config.lr_actor)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(config.state_size, config.action_size, config.random_seed).to(device)
        self.critic_target = Critic(config.state_size, config.action_size, config.random_seed).to(device)
        ...
        # Replay memory
        self.memory = ReplayBuffer(config.action_size, config.buffer_size, config.batch_size, config.random_seed)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        # Learn, if enough samples are available in memory and its time to update
